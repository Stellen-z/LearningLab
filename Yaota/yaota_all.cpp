...
//  依赖顺序排列，各段之间有分隔标注。编译：
//      g++ -std=c++17 -O2 yaota_all.cpp -o yaota
//
//  系统进度：
//    [完成] 五行/境界、随机数、地图生成、物品图鉴(47)、妖怪图鉴(32)、
//           奇遇事件(34)、玩家成长、战斗公式
//    [施工中] 渲染/UI/存档/主循环 —— 文件末尾的 main 是临时自检入口
// ============================================================================
#include <algorithm>
#include <cmath>
#include <cstdlib>
...
public:
    ...
    std::vector<Item>&          spawnedGroundItems() { return groundItems_; }
...
void Dungeon::placeFeatures(int floor, int playerX, int playerY) {
    ...
    for (int i = 0; i < dropCount; ++i) {
        ...
        for (int tries = 0; tries < 20; ++tries) {
            ...
            groundItems_.push_back(Item(Rng::get().pick(eligible)));
            break;
        }
    }
...
// ============================== main.cpp（临时自检入口） ==============================
// 完整游戏主循环（渲染/UI/存档）尚未完成，先用这个 main 验证所有系统可用。

int main() {
#ifdef _WIN32
    system("chcp 65001 > nul"); // Windows 控制台切换到 UTF-8 以显示中文
#endif
    using namespace yaota;
    Rng::get().seed(20260816);

    std::cout << "=== 《妖塔》系统自检 ===\n";
    std::cout << "妖怪图鉴: " << monsterDex().size() << " 种\n";
    std::cout << "物品图鉴: " << itemDex().size() << " 件\n";
    std::cout << "奇遇事件: " << eventDex().size() << " 条\n";

    Player p;
    p.init(L"测试散修", Element::Jin);
    Dungeon d;
    d.generate(5, 2, 2);
    std::cout << "第 5 层生成: " << d.rooms().size() << " 个房间, "
              << d.spawnedMonsters().size() << " 只妖怪, "
              << d.spawnedGroundItems().size() << " 件掉落, 楼梯在 ("
              << d.stairsX() << ", " << d.stairsY() << ")\n";

    Monster m(0, 3, 3);
    m.maxHp = m.hp = monsterDef(0).hp;
    HitResult r = playerHits(p, m);
    std::cout << "试斩野狗妖: ";
    if (r.dodged) std::cout << "被它闪避了\n";
    else          std::cout << "造成 " << r.damage << " 点伤害" << (r.crit ? "（暴击！）" : "") << "\n";
    std::cout << "=== 自检完成，游戏主体施工中 ===\n";
    return 0;
}
